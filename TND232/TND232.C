...
{
  return *(unsigned short __far *)MK_FP(0x40, 0x00 + 2 * (i - 1));  // COM Ports

}

static void usage(void)
{
  cputs("Usage: TND232 [COM1|COM2|COM3|COM4]\r\n"
        "       TND232 STATUS\r\n"
        "       TND232 UNLOAD\r\n");
}
...
int main(void)
{
  ...
  enum mode mode;
  int i;
  cputs("TND232 " XSTR(VERSION_MAJOR) "." XSTR(VERSION_MINOR)
        "  based on TNDLPT github.com/pdewacht/tndlpt\r\n\r\n TND232 - github/matze79/TND232\r\n THX to dreamblaster, pdewacht.\r\n Merry XMAS 2025!\r\n\r\n");
  ...
  for (i = 0; i < 256; ++i) {
    ...
    if (result == 0 && !found_unused_amis_id) {
      ...
      if (info.version.word != (VERSION_MAJOR * 256 + VERSION_MINOR)) {
        cputs("Error: A different version of TND232 is already loaded.\r\n");
        return 1;
      }
      resident =
  ...
  if (mode == MODE_UNLOAD) {
    if (!resident) {
      cputs("TND232 is not loaded.\r\n");
      return 1;
    } else if (uninstall(resident)) {
      cputs("TND232 is now unloaded from memory.\r\n");
      return 0;
    } else {
      cputs("Could not unload TND232.\r\n");
      return 1;
    }
  }
  ...
  tndlpt_port = get_com_port(config.bios_id + 1);
  if (!tndlpt_port) {
    cputs("Error: COM");
  ...
  if (!tndlpt_init(false)) {
    cputs("Error: 232 is not responding\r\n");
    return 1;
  }
